...
class LineFinder:
    ...
    def img_pre_deal(self):
        ...
        cv2.imshow("show_time", edge_img) #show!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        # cv2.waitKey(0)
    ...
    def line_find(self, mode="normal"):
        ...
        if left_line <= 1 and right_line >= 639:
            ...
            flag = "normal"
        return left_line, right_line, mid_line, flag
    ...
    def process(self, mode="cam"):
        ...
        if mode == 'cam':
            ...
            self.img_pre_deal()
            left_line, right_line,loc, flags = self.line_find()
            left_line = left_line / self.image_size[0]
            right_line = right_line / self.image_size[0]
            loc = loc / self.image_size[0]
            return left_line, right_line, loc, flags
        elif mode == "test":
            self.read_image()
            self.img_pre_deal()
...
if __name__ == "__main__":
    line = LineFinder()
    while True:
        line_right, line_left, line_mid, flag = line.process("cam")
        cv2.imshow("img",line.in_doing_img) #show!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        cv2.waitKey(0)
        print(line_mid)
        print(flag)
